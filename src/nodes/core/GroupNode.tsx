 ...
 */
const GroupNode = memo<GroupNodeProps>(({ id, data, selected = false }) => {
  ...
  const handleLabelDoubleClick = useCallback(
    (e: MouseEvent<HTMLSpanElement | HTMLButtonElement>): void => {
      e.stopPropagation();
      setEditValue(label);
      setIsEditing(true);
    },
  ...
  const getColorValue = (c: string): string => {
    if (c.startsWith('var(')) {
    ...
    <>
      ...
      >
        <div className="group-node-header">
          ...
          {childCount > 0 && <span className="group-node-count">{childCount}</span>}
        </div>
