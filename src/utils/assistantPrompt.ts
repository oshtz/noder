...
// Node definition for assistant context
...
// Node types map containing default data with handles
...
const summarizeHandles = (handles: Handle[] = []): HandleSummary => {
  const inputs = handles.filter((handle) => handle.type === 'input' || handle.type === 'target');
  const outputs = handles.filter((handle) => handle.type === 'output' || handle.type === 'source');

  const formatList = (list: Handle[]): string =>
    list.length
      ? list
          .map((handle) => {
...
const truncateText = (value: unknown, maxLen = 120): string => {
...
const formatTemplateNode = (node: TemplateNode = {}): string => {
  if (!node || typeof node !== 'object') return 'unknown';
  const id = node.id || 'node';
  const type = node.type || 'unknown';
  const data = node.data && typeof node.data === 'object' ? node.data : {};
  const previewParts: string[] = [];
  const sanitizePreview = (value: unknown, maxLen: number): string =>
  ...
  if (typeof data.systemPrompt === 'string' && (data.systemPrompt as string).trim()) {
    previewParts.push(`systemPrompt="${sanitizePreview(data.systemPrompt, 80)}"`);
  }
  if (typeof data.model === 'string' && (data.model as string).trim()) {
...
const formatTemplateEdge = (edge: TemplateEdge = {}): string => {
...
const summarizeTemplates = (templates: WorkflowTemplate[] = []): string => {
...
/**
 * Build the system prompt for the AI assistant with workflow context
 */
export const buildAssistantSystemPrompt = ({
  ...
  nodeSchemas,
  workflowTemplates,
}: BuildAssistantSystemPromptParams): string => {
  ...
  return [
    ...
    '',
    '## Using Chips for Reusable Values',
    'Chips let users reuse values across multiple nodes. CRITICAL: Use __CHIPID__ placeholder syntax!',
    '',
    'Example: Character art with consistent style',
    '1. Create chip node: id="style-chip", data: {chipId: "STYLE", content: "anime"}',
    '2. Create image nodes with prompts containing __STYLE__ placeholder:',
    '   - prompt: "Full body character, __STYLE__ art style, detailed"',
    '   - prompt: "Portrait headshot, __STYLE__ art style, expressive"',
    '3. Connect chip output -> each image input',
    '4. When workflow runs, __STYLE__ is replaced with "anime"',
    '',
    'Chip syntax rules:',
    '- chipId should be UPPERCASE (e.g., STYLE, COLOR, CHARACTER)',
    '- Placeholder format is __CHIPID__ with DOUBLE underscores on each side',
    '- The chip content value replaces all __CHIPID__ occurrences in connected prompts',
    '- Connect chip "out" handle to target node "in" handle',
    '',
    '# Error Handling',
    '',
    ...
    '- Use chip nodes when user wants reusable/swappable values (styles, characters, colors)',
    '- When using chips: set chipId in UPPERCASE, use __CHIPID__ in connected prompts',
    '- Only add display-text or markdown when user asks to show output',
    '- Prefer short, stable node ids (text1, img1, chip1, out1)',
