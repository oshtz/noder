...
const getUniqueGradientId = (): string => `edge-gradient-${++gradientIdCounter}`;
...
const getEdgePath = (
  ...
  switch (edgeType) {
    case 'straight':
      return getStraightPath(params) as EdgePathResult;
    case 'step':
      return getSmoothStepPath({ ...params, borderRadius: 0 }) as EdgePathResult;
    case 'smoothstep':
      return getSmoothStepPath({ ...params, borderRadius: 10 }) as EdgePathResult;
    case 'bezier':
    default:
      return getBezierPath(params) as EdgePathResult;
  }
};

interface GlowDimensions {
  rx: number;
  ry: number;
}

const getGlowDimensions = (position: Position): GlowDimensions => {
...
/**
 * CustomEdge - Styled edge with glow effects and data flow animation
 * Features:
 * - Configurable edge types (bezier, straight, step, smoothstep)
 * - Glow effects at connection points
...
const CustomEdge: React.FC<EdgeProps<CustomEdgeData>> = ({
  ...
  const edgeTypeValue: EdgeType = data?.edgeType || 'bezier';
  ...
  return (
    <>
      ...
      {(isProcessing || isDataFlowing) && (
        <defs>
          <linearGradient id={flowGradientId} gradientUnits="userSpaceOnUse">
            <stop offset="0%" stopColor={glowColor as string} stopOpacity={0.2}>
              <animate attributeName="offset" values="-1;1" dur="1.5s" repeatCount="indefinite" />
            </stop>
            <stop offset="50%" stopColor={glowColor as string} stopOpacity={1}>
              <animate
            ...
            <stop offset="100%" stopColor={glowColor as string} stopOpacity={0.2}>
      ...
      {(isProcessing || isDataFlowing) && (
        <circle r="4" fill={glowColor as string} filter="url(#glow)">
