...
export const SchemaForm = ({
  ...
  const handleFieldChange = (field, rawValue) => {
    ...
    if (definition.zod) {
      ...
      setErrors({});
      onChange(parsed.data, { path: field.key, value: parsed.data[field.key], valid: true });
      return;
    }
  ...
  const renderField = (field) => {
    ...
    const isReplicateModel = field.key === 'model' &&
      definition.type?.startsWith('replicate-');

    if (isReplicateModel) {
      // Map node types to Replicate collection slugs
      // These match the actual collection slugs on replicate.com/collections
      // Audio node uses multiple collections (array)
      const collectionMap = {
      ...
      const collectionSlug = collectionMap[definition.type] || null;
    ...
    switch (field.type) {
        ...
        return (
          <div className="schema-field--slider">
            <input
              ...
              step={field.step ?? 0.1}
            />
            <span className="schema-field--value">{values[field.key]}</span>
          </div>
        );
      case 'number':
        return (
      ...
      case 'media-input': {
        ...
        // style_reference and img2img roles both use connected images
    ...
    <div className="schema-form">
      {(definition.fields || []).map((field) => (
        <label key={field.key} className="schema-field">
          ...
          {renderField(field)}
          {errors[field.key] && (
            <div className="schema-field--error">
