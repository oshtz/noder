...
export const HANDLE_TYPES: Record<HandleTypeKey, HandleTypeDefinition> = {
  TEXT: {
    type: 'text',
    dataType: 'text',
    color: '#2196F3', // Blue
...
const TYPE_MAP: Record<string, HandleTypeDefinition> = {
...
export const getHandleColor = (type: string) => {
  return TYPE_MAP[type]?.color || 'var(--primary-color)';
};

// Check if two types are compatible for connection
export const areTypesCompatible = (sourceType: string, targetType: string) => {
...
// Get all valid types
export const getAllTypes = () => Object.keys(TYPE_MAP).filter((key) => key !== 'string');
